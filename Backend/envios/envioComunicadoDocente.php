...
if(isset($data)){
  ...
  try {
      ...
      //si es 0 el envio será para una sola persona
      if ($destinoGrupal==0) {
        $direccion_envio = $emaildestino;
        $mail->addAddress($emaildestino, 'Usted');
      }

       //si es 1 será a sera a una sección en especifico
      if ($destinoGrupal==1) {
       ...
       //si es 2 será para solo los alumnos
      if ($destinoGrupal==2) {
        $direccion_envio = $grado." ".$seccion;
        foreach(emailsAlumnosGradoSeccion($grado,$seccion) as $email) {
          $mail->addAddress($email['email_student'], 'Estudiante');
        }
      }
